...
Frames during training are **teacher-forced**: the field decoder consumes the
ground-truth canonical frame while the frame head is supervised separately. Without
that, early frame error would corrupt the occupancy signal and neither component
would learn. Evaluation reports both teacher-forced and predicted-frame numbers, so
the split is visible in the results rather than hidden in the training loop.
"""
...
class _PerEntityMixer(_nn.Module):
    ...
    def __init__(self, *, width: int, identity_width: int, layers: int, mlp_ratio: float = 4.0) -> None:
        super().__init__()
...
class LagnavPrototype(nn.Module):
    ...
    def __init__(self, config: PrototypeConfig, inverse_relations: torch.Tensor) -> None:
        ...
        self.frame_head = FrameHead(config.entity_width)
    ...
    def forward(
        ...
        *,
        ...
        frames_pred = self.frame_head(entity_latent)
        frames = frames_pred if use_predicted_frames else batch.entity_frames
        present = batch.entity_present

        alignment = self.alignment_entity(entity_latent)

        if self.config.per_entity_geometry:
            tokens = self.tokens(entity_latent, scene_latent, lod=level)
    ...
    def _lod_one_hot(
        self, lod: int, batch_size: int, reference: torch.Tensor
    ) -> torch.Tensor:
